import type { SynthesisBirthSubject } from "../src/lib/western/synthesis/personAdapter.js";
...
interface Body { first: SynthesisBirthSubject; second: SynthesisBirthSubject; query?: TransitQuery }
...
export default async function handler(req: RequestLike, res: ResponseLike) {
  ...
  if (!body || typeof body !== "object" || JSON.stringify(body).length > 12_000) { res.status(400).json({ error: "Invalid relationship overview input" }); return; }
  ...
  try {
    const { resolveWesternRelationshipSynthesisForBirths } = await import("../src/lib/western/synthesis/personAdapter.js");
    const result = resolveWesternRelationshipSynthesisForBirths(candidate.first, candidate.second, candidate.query);
    if (!result.ok) { res.status(422).json({ errors: result.errors }); return; }
    res.status(200).json({ report: result.report });
  } catch (error) { console.error("western relationship overview initialization failed", error); res.status(500).json({ errors: [{ code: "CALCULATION_FAILED", message: "Western relationship overview service failed to initialize" }] }); }
}

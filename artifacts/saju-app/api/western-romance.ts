...
export default async function handler(req: RequestLike, res: ResponseLike) {
  ...
  if (!body || typeof body !== "object" || JSON.stringify(body).length > 4_000) { res.status(400).json({ error: "Invalid birth input" }); return; }
  try {
    const { resolveWesternRelationshipForBirth } = await import("../src/lib/western/interpretation/romance/personAdapter.js");
    const result = resolveWesternRelationshipForBirth(body as WesternBirthSource);
    if (!result.ok) { res.status(422).json({ errors: result.errors }); return; }
    res.status(200).json({ report: result.report });
  } catch (error) {
    console.error("western-romance initialization failed", error);
    res.status(500).json({ errors: [{ code: "CALCULATION_FAILED", message: "Western relationship service failed to initialize" }] });
  }
}

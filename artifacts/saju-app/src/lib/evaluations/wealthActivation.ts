 ...
 *  - 📈 재물 유입 기회도(inflowScore): 방향성 있는 점수. 재성·식상생재·용신/희신 유입은 +,
 *    겁재·재성 뿌리 훼손(충형파해원진)·기신 유입은 -로 반영한다.
 *    ⚠️ 이름 주의: 이 축은 "그 해에 실제 현금이 통장에 들어오는 유불리"가 아니라 "재물로
 *    연결될 수 있는 수익 기회·채널이 열리는 방향"을 뜻한다. 명리에서 재성·식상생재 활성화는
 *    전통적으로 "사업 기회·거래·수익 채널이 열리는 시점" 신호이지 "현금 입금 시점" 신호가
 *    아니며(채널이 열려도 실제 현금화까지는 시차가 있을 수 있음), 이 코드도 재성/식상/용신
 *    희신이 대운·세운에 등장하는지만 보고 실제 현금흐름 데이터를 참조하지 않는다. UI/문구에
 *    "실제 들어오는"이라는 표현을 쓰지 않도록 주의(채널·기회 활성화로 표현할 것).
...
export interface WealthActivationResult {
  /** 재물 유입 "기회"도 — 실제 현금이 그 해에 입금되는지가 아니라, 재성·식상생재·용신희신
   * 활성화로 수익 기회·채널이 열리는 방향(유리/불리)을 뜻한다. 현금화 시점과는 다를 수 있다. */
  inflowScore: number;
...
function buildWealthInterpretation(
  ...
  if (activation === "낮음") {
    return "이 해는 돈과 관련된 사건·거래·기회가 크게 움직이지 않는 평이한 시기입니다.";
  }
  if (activation === "높음" && inflow === "유리" && stability === "안정") {
    return "돈의 움직임이 크고 수익으로 연결될 기회·채널도 열리는 방향이며, 실제로 남기기도 좋은 시기입니다. (기회가 열린다는 뜻이지 현금이 바로 입금된다는 뜻은 아닙니다 — 실제 현금화까지는 시차가 있을 수 있습니다.)";
  }
  if (activation === "높음" && inflow === "불리" && stability === "불안정") {
    return "돈과 관련된 사건·거래는 크게 움직이지만 수익 기회로 연결되는 방향은 약하고 남기기도 어려운 시기 — 지출·손실·분산에 주의가 필요합니다.";
  }
  if (activation === "높음" && stability === "불안정") {
    return "돈의 움직임은 매우 크지만 실제로 남기는 힘은 약한 해입니다. 기회·채널이 열려도 지출·손실·분산으로 빠져나가기 쉬우니 지키는 전략이 필요합니다.";
  }
  if (activation === "높음" && inflow === "불리") {
    return "재물 관련 사건은 크게 움직이는데 수익 기회로 연결되는 방향은 약한 편입니다. 지출·손실 쪽 사건일 가능성에 대비하세요.";
  }
  if (stability === "안정" && activation !== "높음") {
    return "큰 재물 사건·기회는 적지만 기존 자산을 안정적으로 지키기 쉬운 시기입니다.";
  }
  return "재물 활성도, 유입 기회의 방향, 안정·축적도를 함께 참고해서 이 해의 재물 흐름 성격을 판단하세요. 유입 기회도는 실제 현금 입금 시점이 아니라 수익으로 연결될 수 있는 채널이 열리는 방향을 뜻합니다.";
}

// deterministic 문장(synthesizeText 결과)을 AI로 "자연어만" 다듬는 공통 레이어. 계산/
// InterpretationFact 구조는 전혀 바꾸지 않는다 — 이미 완성된 문장을 더 자연스럽게 다시 쓰는
// 후처리 단계일 뿐이다. 새 해석을 추가하지 않도록 서버(api/polish-prose.ts) 프롬프트에서
// 강하게 제한하고, 실패하거나 로그인하지 않은 경우 항상 deterministic 문장을 그대로 쓴다
// (리포트가 절대 깨지지 않는 것을 최우선으로 한다).

...
export function buildCompatibilityClipboardText(
  ...
  // 활성화·조화·충돌·안정적인지를 보여준다.
  {
    ...
    if (pipe1?.evaluations && pipe2?.evaluations) {
      ...
      if (interactionByYear.length > 0) {
        ...
        // 💕 방향성 배우자궁 안정 기여도 — 위 안정도(stability) 점수의 하위 evidence 중
        // 방향이 명확한 것(배우자궁 직접 합·충·형)만 A→B/B→A로 나눠서 보여주는 보조지표.
        // 새 점수 체계가 아니고, 안정도(stability) 자체의 계산에는 전혀 반영되지 않는다.
        const currentYearInteraction = interactionByYear[0];
        if (currentYearInteraction) {
          const ds = currentYearInteraction.result.directionalStability;
          lines.push(`[방향성 배우자궁 안정 기여도(${currentYearInteraction.year}년 기준, 보조지표)]`);
          lines.push(`  위 "안정도(stability)" 점수 전체를 A/B 개인별로 분해한 값이 아닙니다. 안정도를 만드는 근거 중 방향을 분리할 수 있는 것(배우자궁 직접 합·충·형)만 골라 A→B/B→A로 보여주는 보조지표입니다. 양수라고 그 방향의 사람이 상대를 전반적으로 안정시킨다는 뜻이 아니고, 음수라고 관계 전체를 불안정하게 만든다는 뜻도 아닙니다. 0점은 "영향이 없다"가 아니라 "이 시기엔 방향 분리 가능한 직접 배우자궁 근거가 없다"는 뜻입니다.`);
          const describe = (d: typeof ds.aToB) => {
            const scoreLabel = d.hasEvidence ? `${d.score}점` : `0점(근거 없음)`;
            lines.push(`  ${d.fromName} → ${d.toName} 안정 기여도: ${scoreLabel}`);
            if (d.evidence.length > 0) {
              lines.push(`    근거: ${d.evidence.map((e) => `${e.label}(${e.direction === "비우호" ? "-" : "+"}${e.magnitude})`).join(" / ")}`);
            }
          };
          describe(ds.aToB);
          describe(ds.bToA);
  ...
  lines.push("16. 💕 방향성 배우자궁 안정 기여도: [방향성 배우자궁 안정 기여도]는 15번 안정도(stability) 점수 전체를 A/B 개인별로 분해한 값이 아닙니다. 안정도를 만드는 근거 중 방향을 분리할 수 있는 것(배우자궁 직접 합·충·형)만 추려 A→B/B→A로 보여주는 보조지표일 뿐이니, 이 값을 안정도 대신 쓰거나 두 값을 더해서 새로운 총점을 만들지 마세요. 양수라고 그 방향의 사람이 상대를 전반적으로 안정시킨다고 단정하지 말고, 음수라고 관계 전체를 불안정하게 만든다고 단정하지도 마세요. 0점은 '영향이 없다'가 아니라 '이 시기엔 방향 분리 가능한 직접 배우자궁 근거가 없다'는 뜻이니 그렇게 그대로 설명해주세요.");
  lines.push("17. 💕 월별 커플 관계 흐름: [커플 관계 상호작용도(월별)]은 15번(연도별)과 같은 계산에 그 달의 월운만 더한 것입니다. 연도별 흐름과 뭉뚱그리지 말고 구분해서, 그 해 안에서도 어느 달에 관계 활성·조화·안정이 특히 강해지거나 약해지는지처럼 더 세밀한 시기 감각을 보여주는 보조 지표로만 해석해주세요.");
  if (cardPolicy.showRomance) {
    // [Phase 3 P0] 데이터 섹션 이름이 [연애 적합도 / 결혼 적합도]에서 [궁합 점수 (목적별
    // 모델)]로 바뀌었으므로 참조도 함께 갱신한다(더 이상 존재하지 않는 섹션명을 가리키면
    // AI가 근거 없이 지어낼 위험이 있음).
    lines.push("18. 🤝💕💍 인간관계 궁합 · 연애 궁합 · 결혼 궁합: [궁합 점수 (목적별 모델)]의 세 점수는 모두 두 사람 원국 자체의 구조 적합성이며, 연도별 timing과는 완전히 분리된 값입니다. 인간관계 궁합은 '기본적인 상호이해·소통·관계 운영이 구조적으로 얼마나 잘 맞는가'를, 연애 궁합은 '연인으로서 끌리고 관계가 형성·유지되기 쉬운가'를, 결혼 궁합은 '장기 생활·책임·갈등을 운영하기 쉬운가'를 뜻합니다. 세 점수는 서로 다른 산식이며 평균이나 합산으로 만든 값이 아니니 하나로 뭉뚱그리지 말고 각각 구분해서 언급하되, 확률이 아니라 구조적 여건이라는 점을 분명히 하고, 위 연도별·월별 timing 항목(14·15·16·17번)과도 섞지 말아주세요.");
  }
  lines.push("");
  lines.push("────────────────────");
  lines.push("[누락 방지 규칙]");

...
export function buildCompatibilityClipboardText(
    ...
    if (pipe1?.evaluations && pipe2?.evaluations) {
      ...
      if (interactionByYear.length > 0) {
        ...
        if (currentYearInteraction) {
          ...
          lines.push(`  위 "안정도(stability)" 점수를 만드는 근거 중 방향이 명확한 것(배우자궁 직접 합·충·형)만 골라 A→B/B→A로 나눈 값입니다. 별도의 새 점수가 아니라 기존 안정도의 하위 근거를 다시 보여주는 것이며, 0점은 "영향이 없다"가 아니라 "이 방향으로 방향 분리 가능한 근거가 없다"는 뜻입니다.`);
  ...
  lines.push("16. 💕 방향성 배우자궁 안정 기여도: [방향성 배우자궁 안정 기여도]는 15번 안정도(stability) 점수를 만드는 근거 중 방향이 명확한 것만 A→B/B→A로 나눠 보여주는 보조지표이지, 안정도와 별개인 새 점수가 아닙니다. '누가 누구를 더 안정시키는지' 참고용으로만 언급하고, 이 값 자체를 안정도 대신 쓰거나 두 값을 더해서 새로운 총점을 만들지 마세요. 0점은 '영향이 없다'가 아니라 '방향 분리 가능한 근거가 이 시기엔 없다'는 뜻이니 그렇게 그대로 설명해주세요.");
  ...
  if (cardPolicy.showRomance) {
    ...
    lines.push("18. 💕💍 연애 궁합 · 결혼 궁합: [궁합 점수 (목적별 모델)]의 연애 궁합·결혼 궁합 값은 두 사람 원국 자체의 구조 적합성이며, 연도별 timing과는 완전히 분리된 값입니다. 연애 궁합은 '연인으로서 끌리고 관계가 형성·유지되기 쉬운가'를, 결혼 궁합은 '장기 배우자로 생활·책임·갈등을 운영하기 쉬운가'를 뜻합니다. 두 점수와 관계 유형을 함께 언급하되, 확률이 아니라 구조적 여건이라는 점을 분명히 하고, 위 연도별·월별 timing 항목(14·15·16·17번)과 섞어서 하나로 뭉뚱그리지 말아주세요.");
  }
  lines.push("");
  lines.push("────────────────────");
  lines.push("[누락 방지 규칙]");

...
function SectionCard({ section, polishedText }: { section: NatureReportSection; polishedText?: string }) {
  return (
...
export default function ZiweiNatureReport() {
  ...
  const { report, error } = useMemo(() => {
    ...
    if (input.timeUnknown || input.hour === undefined) {
      return { report: null, error: "출생 시간이 없으면 자미두수 명반(命宮·身宮 등)을 정확히 계산할 수 없습니다. 출생 시간을 입력해주세요." };
    }
    ...
    return { report: buildNatureReport(chart, input.name), error: null };
  }, [person]);

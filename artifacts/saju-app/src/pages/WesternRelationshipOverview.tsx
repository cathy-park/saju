...
import { monthInTimezone, monthRange, westernRoutes } from "@/lib/western/uiModel";
...
export default function WesternRelationshipOverview() {
  ...
  useEffect(() => {
    ...
    const timezone = first.birth?.timezone;
    if (!timezone || !second.birth?.timezone) { setState({ errors: [{ code: "MISSING_LOCATION_CONTEXT", field: "location", message: "Both people need explicit Western locations" }], loading: false }); return; }
    const month = monthRange(monthInTimezone(timezone));
    let cancelled = false; setState({ loading: true });
    fetch("/api/western-relationship-overview", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ first: { personId: people.first.id, birth: first.birth }, second: { personId: people.second.id, birth: second.birth }, query: { startLocalDate: month.start, endLocalDate: month.end, timezone } }) })

...
export default function IntegratedOverview() {
  ...
  useEffect(() => {
    ...
    const finish = (western?: WesternPersonalSynthesisReport, westernChart?: WesternPersonalityReport["chart"]) => { if (cancelled) return; const scope = month ? monthScope(month) : undefined; setReport(buildIntegratedPersonalReport({ personId: person.id, sources: [...sources, ...(western ? adaptWesternPersonal(western, scope) : [])], ...(scope ? { selectedPeriod: { start: scope.start, end: scope.end, timezone: scope.timezone } } : {}) })); setCopyPrompt(buildIntegratedCopyPrompt({ saju: buildPersonClipboardText(person, "포함", true), ziwei: existing.ziweiChart ? buildZiweiCopyPrompt(existing.ziweiChart) : "출생시간이 없어 명반을 계산하지 않았습니다.", western: westernChart ? buildWesternCopyPrompt(westernChart, { placeLabel: person.westernLocation?.placeLabel }) : "출생지 좌표와 timezone을 설정하면 계산 구조가 포함됩니다." })); };
    ...
    Promise.all([fetch("/api/western-overview", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ personId: person.id, birth, ...(scope ? { query: { startLocalDate: scope.start, endLocalDate: scope.end, timezone: birth.timezone } } : {}) }) }), fetch("/api/western-personality", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(birth) })]).then(async ([overviewResponse, chartResponse]) => { if (!overviewResponse.ok || !chartResponse.ok) throw new Error(); const overview = await overviewResponse.json() as { report: WesternPersonalSynthesisReport }; const chart = await chartResponse.json() as { report: WesternPersonalityReport }; finish(overview.report, chart.report.chart); }).catch(() => finish());

...
export default function WesternSynastry() {
  ...
  return <WesternReportShell personId={people.first.id} sajuHref={people.first.id === getMyProfile()?.id ? "/saju" : `/people/${people.first.id}`} westernHref={westernRoutes.relationship(people.first.id, people.second.id).overview} eyebrow="서양점성술 · 시너스트리" title={`${people.first.birthInput.name}님과 ${people.second.birthInput.name}님의 관계`} description="점수나 사건 예측 없이 두 출생차트가 실제로 만나는 방식을 봅니다." navigation={<WesternRelationshipNav personId={people.first.id} otherPersonId={people.second.id} />}>

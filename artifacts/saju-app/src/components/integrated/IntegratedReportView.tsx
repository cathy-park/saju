...
import { buildIntegratedCopyPrompt, buildHolisticDeterministicText, polishIntegratedHolistic, polishIntegratedSection } from "@/lib/integrated/prompt";
...
export function IntegratedReportView({ report }: { report: IntegratedReport }) {
  ...
  // 21단계 — "종합" 화면의 최종 표현은 섹션별 개별 다듬기(polishIntegratedSection, 아래
  // 근거 계층에서 계속 씀)를 넘어, 리포트 전체를 한 사람/관계를 설명하는 하나의 상담문으로
  // 통합한 holistic AI 결과다. deterministic 문장(buildHolisticDeterministicText)은 그대로
  // 두고, AI가 실패하거나 로그인하지 않았으면 그 문장이 fallback으로 그대로 보인다.
  ...
  return <div className="ds-section-gap">
    ...
    <section className="ds-card ds-card-pad shadow-none border-primary/20 bg-primary/[0.03]">
      <p className="text-xs font-bold uppercase tracking-widest text-primary">종합 상담</p>
      <p className="mt-2 text-sm leading-relaxed text-foreground">{holistic}</p>
    </section>
    <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">체계별 확정 근거</p>
